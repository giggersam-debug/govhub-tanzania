    ...
    <>
      <div className="bg-paper border-b border-line py-8">
        <div className="max-w-[1140px] mx-auto px-6">
          <Breadcrumb items={[[t.breadcrumbHome, "/"], [t.breadcrumbAgencies, "/agencies"], [a.code, null]]} />
      ...
      <section className="max-w-[1140px] mx-auto px-6 py-11">
        <h2 className="font-display text-2xl mb-5">{t.formsFromPrefix} {a.code}</h2>
        <div className="flex flex-col gap-2.5">
          {formList.length ? (
            formList.map((f) => <FormRow key={f.id} form={f} agencyCode={a.code} lang={lang} />)
          ) : (
            <p className="text-inksoft text-sm">{t.noFormsForAgency}</p>
          )}
        </div>
      </section>
    </>
